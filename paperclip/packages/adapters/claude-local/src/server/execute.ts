...
/**
 * Resolve AZOTH proxy settings from adapter config or server environment variables.
 * Priority: config.azothBaseUrl > PAPERCLIP_AZOTH_BASE_URL env var.
 */
function resolveAzothConfig(config: Record<string, unknown>): {
  baseUrl: string | null;
  apiKey: string | null;
} {
  const baseUrl =
    (typeof config.azothBaseUrl === "string" && config.azothBaseUrl.trim()) ||
    process.env.PAPERCLIP_AZOTH_BASE_URL?.trim() ||
    null;
  const apiKey =
    (typeof config.azothApiKey === "string" && config.azothApiKey.trim()) ||
    process.env.PAPERCLIP_AZOTH_API_KEY?.trim() ||
    null;
  return { baseUrl: baseUrl || null, apiKey: apiKey || null };
}
...
function applyAzothRouting(
  ...
  if (azoth.baseUrl) {
    env.ANTHROPIC_BASE_URL = azoth.baseUrl;
    // AZOTH manages the actual API key; agents see only the proxy.
    if (azoth.apiKey) {
      env.ANTHROPIC_API_KEY = azoth.apiKey;
    }
  }
